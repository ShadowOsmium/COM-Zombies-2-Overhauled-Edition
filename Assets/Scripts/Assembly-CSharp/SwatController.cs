...
public class SwatController : PlayerController
{
	public const string ANI_GRENADE = "UpperBody_Avatar_Swat_Skill01";
	...
	{
		base.Awake();
		GRENADE_SKILL_STATE = PlayerState.Create(PlayerStateType.Grenade, this);
		...
		base.GetComponent<Animation>()["UpperBody_Avatar_Swat_Skill01"].AddMixingTransform(spine);
	}
	...
	{
		SkillGrenadeController skillGrenadeController = skill_set["Grenade"] as SkillGrenadeController;
		float num = 0f;
		float num2 = 10f;
		float num3 = 12f;
		float num4 = num2 / num3;
		float num5 = (num - 0.5f * Physics.gravity.y * num4 * num4) / num4;
		Vector3 vector = Vector3.up * num5 + Camera.main.transform.forward * num3;
		...
		component.explode_radius = skillGrenadeController.skill_data.range_val;
		component.damage = skillGrenadeController.skill_data.damage_val;
