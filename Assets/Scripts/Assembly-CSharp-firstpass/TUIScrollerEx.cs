...
public class TUIScrollerEx : TUIControlImpl
{
	...
	public bool wait_scroll;

	public Action OnScrollMove;

	public Action OnScrollClick;

	public ScrollerEnabel OnScrollerEnabel;
	...
	{
		if (OnScrollerEnabel != null && !OnScrollerEnabel())
		{
			return false;
		}
		if (input.inputType == TUIInputType.Began)
		{
			if (PtInControl(input.position) && (!wait_scroll || (wait_scroll && !scroll)))
			{
	...
	public void SimScroll(Vector2 speed)
	{
		moveSpeed = speed;
		StartScroll();
	}
